...
int editor_update_internal(struct EditorMemory *mem, struct EditorCtx *ectx, struct FrameContext *frame)
{
    ...
    editor_get_window_size(frame, &window_width, &window_height);
    ...
    camera_compute_view(&ectx->camera, editor_get_window_ratio(frame));
    GLCheckError();
    ...
    if(igIsMouseDown_Nil(ImGuiMouseButton_Middle))
    {
        ...
        {
            float pan_x, pan_y;
            pan_x = -mouse_delta_x*2/ectx->camera.zoom*editor_get_window_ratio(frame)/(float)window_width;
...
{
    return frame->window.ratio;
}

void editor_get_window_size(struct FrameContext *frame, int *w, int *h)
{
    *w = frame->window.width;
    *h = frame->window.height;
}
