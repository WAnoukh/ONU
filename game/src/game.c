...
void update_key_blocks(struct Game *game, struct InputInfo inputs)
{
    struct GameState *gamestate = get_current_gamestate(game);
    ...
    for(int i =0; i < gamestate->key_block_data_count; ++i)
    {
        ...
        if(is_key_pressed(inputs, key_data->key))
        {
        ...
        any_non_universal_key_down |= is_key_down(inputs, key_data->key);
    }
    ...
    for(int i = 0; i < gamestate->entity_count; ++i)
    {
        ...
        {
            key_data->is_pressed = is_key_down(inputs, key_data->key);
            key_pressed = is_key_pressed(inputs, key_data->key);
        }
        if(key_pressed )
        {
            if(has_level_views && !key_data->is_global)
...
void game_update(struct Game *game, struct FrameContext* frame)
{
    ...
    {
        camera_compute_view(&game->camera, frame->window.ratio);
    }

    update_key_blocks(game, frame->inputs);
